...
- Read and append execution history in `.tdd_activitylog.md` instead of storing detailed history inside `.tdd_protocol.md`.
...
- If suitable documentation does not yet exist under `docs/`, create or extend a concise document there before implementation. Do not overload `.tdd_protocol.md` with long-lived design content or detailed chronological logs.
...
- Keep `.tdd_protocol.md` concise. Put chronological execution entries in `.tdd_activitylog.md` and append new entries there for every task.
...
- After any task, always update `.tdd_protocol.md` and append the execution history to `.tdd_activitylog.md`.
...
- When `Status` changes, update `.tdd_protocol.md` in the same task where the triggering event occurred and record the reason in `.tdd_activitylog.md` so the state transition is auditable.
- If requirements/basic design/detailed design were clarified during the task, reflect those changes in the appropriate `docs/` files before or together with code/test changes, and keep `.tdd_protocol.md` limited to the active objective, execution tasks, status, and references.
- For `bd` and `dt`, restrict edits to `docs/` files plus the minimum `.tdd_protocol.md` synchronization needed to record state.
- Do not dump the full updated `.tdd_protocol.md` after command execution unless the user explicitly asks for the file contents.
- If there is a concrete next command the user should run, propose it at the end of the response with the label `推奨:`.